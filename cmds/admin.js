...
function default_1({ event, botData, api, getThread }) {
    ...
    users.forEach((id) => {
        ...
        id = parseInt(id);
        (getThread.admins.includes(id)) ? getThread.admins.splice(getThread.admins.indexOf(id), 1) : getThread.admins.push(id);
        api.sendMessage({
            body: `Đã ${getThread.admins.includes(id) ? 'thêm' : 'gỡ'} admin: ${name}.`,

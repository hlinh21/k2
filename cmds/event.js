...
function default_1({ event, api, loadedEvents }) {
    if (event.args[0] == 'count')
        ...
        if (event.args[1] == 'all') {
            const eventFiles = fs_extra_1.readdirSync(`${__dirname}/../events`).filter(item => item.endsWith('.ts') && !item.includes('test'));
            for (let file of eventFiles) {
                try {
                    ...
                    return console.log(`Không thể tải event "${file.replace('.ts', '')}" vì đã có lỗi xảy ra:\n${err}`);
                }
            }

...
async function default_1({ event, botData, api, getThread, botID, threadAdmins }) {
  ...
  if(!args[0]){
    ...
    if (isNaN(event.args[1])) return out('Hãy nhập số thứ tự của rule cần xóa.');
      let getRule = getThread.rule[parseInt(args[1]) - 1];
            if (!getRule) return out(`Không có shortcut mang số thứ tự ${event.args[0]}.`, event.threadID, event.messageID);
            getThread.rule.splice(getThread.rule.indexOf(getRule), 1);
            api.sendMessage(`Đã xóa rule có nội dung: ${getRule}.`, event.threadID, event.messageID);
    }
